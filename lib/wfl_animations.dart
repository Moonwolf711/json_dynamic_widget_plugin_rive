...
final nigelIdleTween = MovieTween()
  ...
  ..scene(begin: Duration.zero, duration: const Duration(milliseconds: 4000))
      .tween('breathY', Tween<double>(begin: 0.0, end: 2.5), curve: Curves.easeInOut)
  // Eye wander X - lazy drift (5 second cycle)
  ..scene(begin: const Duration(milliseconds: 300), duration: const Duration(milliseconds: 5000))
      .tween('eyeX', Tween<double>(begin: -2.0, end: 2.0), curve: Curves.easeInOut)
  // Eye wander Y - even slower (6 second cycle)
  ..scene(begin: const Duration(milliseconds: 800), duration: const Duration(milliseconds: 6000))
      .tween('eyeY', Tween<double>(begin: -1.2, end: 1.2), curve: Curves.easeInOut)
  // Head bob - chill (1.2 second cycle)
  ..scene(begin: Duration.zero, duration: const Duration(milliseconds: 1200))
      .tween('headBob', Tween<double>(begin: 0.0, end: 2.0), curve: Curves.easeInOut)
  // Body sway - relaxed (3.5 second cycle)
  ..scene(begin: Duration.zero, duration: const Duration(milliseconds: 3500))
      .tween('sway', Tween<double>(begin: -2.8, end: 2.8), curve: Curves.easeInOut)
  // Lean - glacial rotation (8 second cycle)
  ..scene(begin: Duration.zero, duration: const Duration(milliseconds: 8000))
      .tween('lean', Tween<double>(begin: -1.5, end: 1.5), curve: Curves.easeInOut);

/// Button glow animation for UI elements
final buttonGlowTween = MovieTween()
  ..scene(begin: Duration.zero, duration: const Duration(milliseconds: 2500))
      .tween('glow', Tween<double>(begin: 0.3, end: 0.7), curve: Curves.easeInOut);
