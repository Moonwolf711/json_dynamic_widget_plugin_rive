...
class _WFLAnimatorState extends State<WFLAnimator>
  ...
  // SFX Panel
  bool _sfxPanelExpanded = false; // Collapsed by default

  // ============ ACTION DISPATCHER SYSTEM ============
  late final ButtonState _uiButtonState;
  late final WFLActionDispatcher _actionDispatcher;
  ...
  void initState() {
    ...
    // Initialize action dispatcher for unified button handling
    _uiButtonState = ButtonState();
    _actionDispatcher = WFLActionDispatcher(
      baseUrl: 'http://localhost:8080', // TODO: configure from WFLConfig
      state: _uiButtonState,
      onSfxPlay: _playSfx,
      onStateMachineTransition: _handleStateMachineTransition,
      onWsEmit: (event, payload) {
  ...
  Future<CommandResult> animateCharacter(
    ...
    try {
      ...
      return CommandResult.success('G�� Animated $character: $animation');
    } catch (e) {
  ...
  Future<CommandResult> setMouthShape(String character, String shape) async {
    try {
      ...
      return CommandResult.success('G�� Set $character mouth: $shape');
    } catch (e) {
  ...
  Future<CommandResult> triggerBlink(String character) async {
    try {
      if (character == 'both') {
        _triggerBlink('terry');
        _triggerBlink('nigel');
        return CommandResult.success('G�� Both characters blinked');
      } else {
        _triggerBlink(character);
        return CommandResult.success('G�� $character blinked');
      }
    } catch (e) {
  ...
  Future<CommandResult> playSFX(String sfxName) async {
    try {
      ...
      return CommandResult.success('G�� Played SFX: $sfxName');
    } catch (e) {
  ...
  void _onRiveInit(Artboard artboard) {
    ...
    if (_terryController?.hasDataBinding == true) {
      debugPrint('G�� Using Data Binding API for lip-sync');
    } else {
      debugPrint('G�� Data Binding not found, using legacy PNG sprites');
    }
  }
  ...
  void _toggleShowMode() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('=�ļ SHOW MODE ON - Terry & Nigel are watching...'),
  ...
  /// Hit test cockpit tap G�� find button + re-focus keyboard
  ...
  /// Press button: flash G�� on, play character grunt
  Future<void> _pressButton(String btn) async {
    // Toggle state
    final currentState = _buttonStates[btn] ?? 0;
    final newState = currentState == 0 ? 2 : 0; // off G�� on
    ...
    try {
      ...
      if (!controller.value.isPlaying && controller.value.hasError) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Video skipped G�� convert to MP4 (H.264) first'),
  ...
  List<MouthCue> _generateMouthCues(String text) {
    ...
    for (int i = 0; i < text.length; i++) {
      ...
      if ('a+�+�'.contains(char)) {
        ...
        mouth = 'e';
      else if ('i+�+�'.contains(char))
        mouth = 'i';
      else if ('o+�+�'.contains(char))
        mouth = 'o';
  ...
  Widget _exportOption(BuildContext ctx, ExportPreset preset) {
    return ListTile(
      ...
      subtitle: Text('${preset.resolution} G�� ${preset.description}',
          style: const TextStyle(color: Colors.grey, fontSize: 11)),
      onTap: () => Navigator.pop(ctx, preset),
    );
  }

  /// Export & Post - one click: render G�� YouTube G�� share sheet
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Rendering G�� YouTube G�� Share Sheet',
                style: TextStyle(color: Colors.grey)),
          ],
        ),
      ),
  ...
  /// Build the SFX trigger buttons panel using action dispatcher
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              const Icon(Icons.music_note, color: Colors.cyan, size: 18),
              const SizedBox(width: 8),
              const Text(
                'SFX',
                style: TextStyle(
  ...
  /// Handle state machine transitions from action dispatcher
  void _handleStateMachineTransition(String stateName) {
    debugPrint('State Machine: Transitioning to $stateName');
    switch (stateName) {
      case 'idle':
        _terryAnimation = 'idle';
        _nigelAnimation = 'idle';
        break;
      case 'talking':
        _terryAnimation = 'talking';
        _nigelAnimation = 'talking';
        break;
      case 'laughing':
        _terryAnimation = 'laughing';
        _nigelAnimation = 'laughing';
        break;
      case 'surprised':
        _terryAnimation = 'surprised';
        _nigelAnimation = 'surprised';
        break;
      default:
        debugPrint('Unknown state: $stateName');
    }
    setState(() {});
  }
