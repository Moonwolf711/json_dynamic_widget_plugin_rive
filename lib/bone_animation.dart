...
class Skeleton {
  ...
  Bone? _cachedRootBone;

  /// Get bone by name (cached)
  Bone? getBone(String name) {
    _bonesByName ??= {for (final b in bones) b.name: b};
    return _bonesByName![name];
  }

  /// Get children of a bone (cached)
  List<Bone> getChildren(String parentName) {
    if (_childrenByParent == null) {
      _childrenByParent = {};
      for (final bone in bones) {
        final parent = bone.parentName ?? '';
        _childrenByParent!.putIfAbsent(parent, () => []).add(bone);
      }
    }
    return _childrenByParent![parentName] ?? const [];
  }

  /// Get root bone (cached)
  Bone? get rootBone {
    if (_cachedRootBone == null && bones.isNotEmpty) {
      for (final bone in bones) {
        if (bone.parentName == null) {
          _cachedRootBone = bone;
          break;
        }
      }
...
class BoneAnimatorWidgetState extends State<BoneAnimatorWidget>
  ...
  void _buildImageCache() {
    _imageCache.clear();
    for (final bone in widget.skeleton.bones) {
      for (final imagePath in bone.attachedImages) {
        final fullPath = '${widget.assetBasePath}/$imagePath';
        if (!_imageCache.containsKey(fullPath)) {
          _imageCache[fullPath] = Image.asset(
            fullPath,
            errorBuilder: (_, __, ___) => const SizedBox(),
            gaplessPlayback: true, // Prevents flicker during animation
          );
        }
      }
    }
